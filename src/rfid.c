...
card_rfid  *head;
/* finds holder name by card UID*/
char* find(  char* a)
{
    card_rfid *curr = head;

    while (curr != NULL) {
        if (!strcmp(a, curr->uidArray)) {
            return curr->user;
        }
...
void  rfid_add_card( card_rfid *card)
{
    ...
    //copying data
    card_rfid *new_card = (malloc(sizeof(card_rfid) + 1));
    char *user = malloc(strlen(card->user) + 1);
    strcpy(user, card->user);
    new_card->user = user;
    char *size = malloc(strlen(card->size));
    strcpy(size, card->size );
    new_card->size = size;
    char *uidArray = malloc(strlen(card->uidArray) + 1);
